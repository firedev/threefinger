...
if [ "$replaced" = 1 ]; then
    cat <<EOF
     Upgrading from 1.1.x: the old threefinger entry no longer works even if it
     shows as on (1.2.0+ keeps the grant across upgrades). Select it, click −, then + → $BINDIR/threefinger.
EOF
fi
cat <<'EOF'
